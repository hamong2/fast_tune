...
class FastSurferVINN(FastSurferCNNBase):
    ...
    * Loss Function (weighted Cross-Entropy and dice loss)
    ...
    def forward(self, x, scale_factor, scale_factor_out=None):
        ...
        outblock = self.outp_block(decoder_output0, skip_encoder_0) # (8,71,320,320) in filter shape -> out class로 바꾸어주는

        # Final logits layer
        logits = self.classifier.forward(outblock)  # 1x1 convolution
        # print("@@@@@@@@@@@@\n In networks LogitSHAPE", logits.shape) : (8, 79, 320, 320)
        # print(logits[1, 30, 150:170, 150:170])
